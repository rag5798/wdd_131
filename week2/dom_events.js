...
function renderTasks(tasks) {
  ...
  const html_nodes = tasks.map(task => {
  ...
  if (task.completed) {
    list_element.classList.add('strike');
  }
  return list_element
});

  html_nodes.forEach(element => {
...
function newTask() {
  ...
  const user_input = document.querySelector('#todo');
  const json = {detail: user_input.value, completed: false};
...
function removeTask(taskElement) {
  ...
  taskElement.remove();
  console.log(`Removed ${taskElement}`)
}
...
const enter = document.querySelector('#submitTask')
