...
recipes.forEach(createRecipeBox)
...
function createRecipeBox(object) {
    ...
    img.alt = `${object.name} image that leads to ${object.url}`;
    img.className = 'recipe-images';
    ...
    object.tags.forEach(element => {
        ...
        tag1.className = 'tag1';
        tag1.textContent = element;
        typeDiv.appendChild(tag1);
    });
    ...
    ratingDiv.appendChild(itemName);

    const stars = document.createElement('p');
    stars.className = 'stars';
    let starString = '';

    for (let i = 0; i < Math.floor(object.rating); i++) {
        starString += '&#9733;';
    }

    
    for (let i = Math.floor(object.rating); i < 5; i++) {
        starString += '&#9734;';
    }
...
function search(query) {
    const main = document.querySelector('main');
    const allSections = main.querySelectorAll('.recipe-box');
	const filteredSections = []
    
	allSections.forEach(section => {
		const tags = section.querySelectorAll('.tag1')
		const name = section.querySelector('.item-name')

		tags.forEach(tag => {
			if (tag.textContent.toLowerCase().includes(query) || name.textContent.toLowerCase().includes(query)){
